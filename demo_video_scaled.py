...
if __name__ == '__main__':
    ...
    path = "/scratch/aparna/ASL_preprocess/"
    for  folder in os.listdir(path):
        for video_path in os.listdir(os.path.join(path,folder)):
            out_path = os.path.join(path,folder,video_path,'flame_init', 'flame_params')
            if os.path.exists(out_path) :
                continue
            os.makedirs(out_path, exist_ok=True)
            input_path = os.path.join(path,folder,video_path,"video.mp4")
            ...
            flame_shape_param =[]
            while True:
                tqdm.write(f'Processing frame {frame_idx}')
                ret, image = cap.read()
                ...
                root_pose = outputs['pose_params'].view(3).tolist()
                jaw_pose = outputs['jaw_params'].view(3).tolist()
                neck_pose, leye_pose, reye_pose = [0,0,0], [0,0,0], [0,0,0]
                expr = outputs['expression_params'].view(-1).tolist()
                flame_shape_param.append(outputs['shape_params'].view(-1))
                cam = outputs['cam'].view(3).tolist()
                is_valid = True
                flame_param = {'root_pose': root_pose, 'neck_pose': neck_pose, 'jaw_pose': jaw_pose, 'leye_pose': leye_pose, 'reye_pose': reye_pose, 'expr': expr, 'is_valid': is_valid}
                with open(osp.join(out_path, str(frame_idx) + '.json'), 'w') as f:
                    json.dump(flame_param, f)
                
                os.makedirs(osp.join(path,folder,video_path, 'flame_init', "cam"), exist_ok=True)
                with open(osp.join(path,folder,video_path, 'flame_init', "cam", str(frame_idx) + '.json'), 'w') as f:
                    json.dump(cam, f)

                frame_idx += 1
            flame_shape_param = torch.stack(flame_shape_param).mean(0).tolist()
            with open(osp.join(path,folder,video_path, 'flame_init', 'shape_param.json'), 'w') as f:
